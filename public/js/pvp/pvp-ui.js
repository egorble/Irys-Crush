...
function showPVPMainInterface() {
    ...
    // Перевіряємо чи є активна гра, яку можна відновити
    if (window.pvpTimePersistence) {
        // Перевіряємо всі можливі кімнати в localStorage
        const gameInfo = checkForActiveGame();
        if (gameInfo) {
            showActiveGamePrompt(gameInfo);
            return;
        }
    }
...
function checkForActiveGame() {
    if (!window.pvpTimePersistence) return null;
    
    try {
        const saved = localStorage.getItem('pvp_game_time_state');
        ...
        if (timeState.savedAt < hourAgo) {
            window.pvpTimePersistence.clearTimeState();
            return null;
        }

        const gameInfo = window.pvpTimePersistence.getGameInfo(timeState.roomId);
        if (gameInfo && gameInfo.isValid && gameInfo.timeRemaining > 0) {
            return gameInfo;
        }
    } catch (error) {
...
function showActiveGamePrompt(gameInfo) {
    const pvpContent = document.querySelector('#pvp-modal .modal-content');
    if (!pvpContent) return;

    // Формуємо інформацію про час з урахуванням паузи
    let timeInfo = `
        <p><strong>Time Remaining:</strong> ${gameInfo.formattedTimeRemaining}</p>
        <p><strong>Active Time:</strong> ${gameInfo.formattedActiveTime}</p>
    `;
    
    if (gameInfo.totalPausedTime > 0) {
        timeInfo += `<p><strong>Paused Time:</strong> ${gameInfo.formattedPausedTime}</p>`;
    }

    const pauseStatus = gameInfo.isPaused ? 
        '<p class="pause-status paused">⏸️ Game is currently paused</p>' : 
        '<p class="pause-status active">▶️ Game was active when saved</p>';

    pvpContent.innerHTML = `
        <div class="active-game-prompt">
            <h3>🎮 Active Game Found!</h3>
            <div class="game-info">
                <p><strong>Room:</strong> ${gameInfo.roomId}</p>
                ${timeInfo}
                <p><strong>Current Score:</strong> ${gameInfo.score}</p>
                ${pauseStatus}
            </div>
            <div class="prompt-actions">
                <button id="resume-game-btn" class="pvp-btn primary">
                    🔄 Resume Game
                </button>
                <button id="abandon-game-btn" class="pvp-btn danger">
                    🗑️ Abandon Game
                </button>
            </div>
            <p class="info-text">
                ℹ️ Your game time was paused while you were away. Resume to continue playing!
            </p>
            <p class="warning-text">
                ⚠️ If you abandon the game, you will lose your entry fee and the game will be forfeited.
            </p>
        </div>
    `;

    // Add event listeners
    document.getElementById('resume-game-btn').onclick = () => resumeActiveGame(gameInfo.roomId);
    document.getElementById('abandon-game-btn').onclick = () => abandonActiveGame(gameInfo.roomId);
}

async function resumeActiveGame(roomId) {
    try {
        console.log('🔄 Resuming active game for room:', roomId);
        
        // Запускаємо гру з відновленням стану
        if (window.startPVPGame) {
            await window.startPVPGame(roomId, 2, false); // gameTime буде відновлено з localStorage
        } else {
            throw new Error('PVP Game system not loaded');
        }
    } catch (error) {
        console.error('❌ Failed to resume game:', error);
        if (window.showNotification) {
            window.showNotification('Failed to resume game: ' + error.message, 'error');
        }
        showPVPMainInterface();
    }
}

function abandonActiveGame(roomId) {
    if (!confirm('Are you sure you want to abandon the active game? You will lose your entry fee.')) {
        return;
    }

    console.log('🗑️ Abandoning active game for room:', roomId);
    ...
    if (window.showNotification) {
        window.showNotification('Game abandoned', 'info');
    }
...
async function refreshRoomInfo(roomId) {
    ...
    try {
        ...
        // Update host join status and leave button
        const hostJoinStatus = document.getElementById('host-join-status');
        const joinRoomBtn = document.getElementById('join-room-btn');
        const leaveRoomBtn = document.getElementById('leave-room-btn');
        
        if (isHost && !isUserInRoom && hostJoinStatus && joinRoomBtn) {
            // Host hasn't joined yet - show join button and cancel option
            hostJoinStatus.style.display = 'block';
            ...
            if (hostJoinStatus) hostJoinStatus.style.display = 'none';
        ...
        if (startGameBtn && isHost) {
            ...
            if (!isUserInRoom) {
                startGameBtn.textContent = '⚠️ Join room first to start game';
                startGameBtn.style.background = '#ff9800';
            } else if (canStart) {
                startGameBtn.textContent = `🎮 Start Game (${room.currentPlayers} players)`;
                startGameBtn.style.background = 'linear-gradient(45deg, #4CAF50, #45a049)';
            } else if (room.gameStarted) {
