...
contract IrysCrushLeaderboard {
    ...
    /**
     * @dev Submit game results from server (ONLY GAME SERVER)
    ...
    function submitGameResults(
        ...
        if (room.gameFinished) revert("Game already finished");
        if (_players.length != _scores.length) revert("Arrays length mismatch");
    ...
    /**
     * @dev Internal function to distribute rewards when server finishes game
    ...
    function _distributeServerRewards(uint256 _roomId) internal {
        ...
        emit GameFinished(_roomId, winner, winnerPrize);
    }
